<template>
  <div class="travelRequestpage" ref="travelRequestpage">
    ...
    <template v-if="isNew">
      ...
      >
        <div>
          <span style="font-size: 18px;font-weight: bold;">出差地点</span>
        </div>
        <div style="padding-top:10px">
          <textarea type="text" class="leaveReasons" placeholder="请输入出差地点" v-model="travelAddress"></textarea>
        </div>
      </div>
      <div
        type="primary"
      ...
      >
        <div>
          <span style="font-size: 18px;font-weight: bold;">出差事由</span>
          <img
            class="imge"
        ...
        <div style="padding-top:10px">
          <textarea type="text" class="leaveReasons" placeholder="请输入出差事由" v-model="leaveReasons"></textarea>
        </div>
...
export default {
  ...
  methods: {
    ...
    deleteImg: function(index) {
      ...
      this.$defines.setTravelRequestImges(this.imgs);
      this.$defines.setTravelRequestFileData(this.fileData);
    },
    //图片click
    imgClick: function() {
    ...
    readLocalFile: function() {
      ...
      reader.onload = function(event) {
        ...
        current.$defines.setTravelRequestImges(current.imgs);
      };
    ...
    //新增
    pendingTrial() {
      if (localStorage.getItem("travelRequestApproved") == "true") {
        return;
      } else {
        ...
        localStorage.setItem("travelRequestApproved", true);
        this.isNew = true;
        this.getListData();
      }
    },
    examined() {
      if (localStorage.getItem("travelRequestApproved") == "false") {
        ...
        localStorage.setItem("travelRequestApproved", false);
        this.isNew = false;
        this.getListData();
      }
    },
    getListData() {
      if (!this.isNew){
        ...
        let urls =
          ...
          "/user/searchApprovalBusinessTraveI.do";
        _this.$ajax
          ...
          .then(function(response) {
            ...
            _this.approvalList = returnData.data.businessTraveIRecordList;
          });
      }
    },
    getDay(date, num) {
    ...
    getAttendanceRecord() {
      ...
      for (let i = 0; i < this.$defines.travelRequestFileData.length; i++) {
        let file = this.$defines.travelRequestFileData[i].files[0];
    ...
    sure() {
      ...
      var url =
        ...
        "/" +
        this.getPROJECT_MAIN() +
        "/user/addBusinessTraveIRecord.do";
    ...
    changeFixed(clientHeight) {
      //动态修改样式
      this.$refs.travelRequestpage.style.height = clientHeight + "px";
    },
    goLeaveRequestInformation(item) {
      this.$router.push({
        path: "/travelRequestInformation",
        query: {
          pagename: "travelRequestpage",
          item: item
        }
  ...
  watch: {
    ...
    clientHeight: function() {
      this.totalHeight = this.$refs.travelRequestpage.offsetHeight;
  ...
  created: function() {
    ...
    if (this.$route.query.pagename == "travelRequestInformation") {
      localStorage.setItem("travelRequestApproved", false);
      _this.isNew = false;
