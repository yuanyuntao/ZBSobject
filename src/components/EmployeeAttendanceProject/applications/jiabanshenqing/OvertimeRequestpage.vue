...
export default {
  ...
  methods: {
    //监听返回按钮
    goBack() {
      this.$defines.setOvertimeRequestImges([]);
      this.$defines.setOvertimeRequestFileData([]);
      this.$router.push({
        path: "/application",
        query: {
    ...
    deleteImg: function(index) {
      ...
      this.$defines.setOvertimeRequestImges(this.imgs);
      this.$defines.setOvertimeRequestFileData(this.fileData);
    },
    //图片click
    imgClick: function() {
    ...
    readLocalFile: function() {
      ...
      reader.onload = function(event) {
        ...
        current.$defines.setOvertimeRequestImges(current.imgs);
      };
      ...
      if (this.isNew) {
        ...
        _this.$ajax
          ...
          .then(function(response) {
            ...
            _this.options = returnData.data.overtimeType;
          });
        //休假时长
        // var content = {
        //   userId: _this.userId
        ...
        let urls =
          ...
          "/" +
          _this.getPROJECT_MAIN() +
          "/user/searchApprovalOvertime.do";
        _this.$ajax
          .post(urls, headerAndBody.contentDataByKey, {
            headers: {
              appEncryptedKey: headerAndBody.appEncryptedKey, //使用服务器RSA公钥加密后的AES密钥
          ...
          .then(function(response) {
            ...
            _this.approvalList = returnData.data.overtimeRecordList;
          });
      }
    },
    getDay(date, num) {
    ...
    getAttendanceRecord() {
      ...
      for (let i = 0; i < this.$defines.overtimeRequestFileData.length; i++) {
        let file = this.$defines.overtimeRequestFileData[i].files[0];
    ...
    goLeaveRequestInformation(item) {
      this.$router.push({
        path: "/overtimeRequestInformation",
        query: {
          pagename: "overtimeRequestpage",
          item: item
        }
  ...
  created: function() {
    ...
    if (this.$route.query.pagename == "overtimeRequestInformation") {
      localStorage.setItem("overtimeRequestApproved", false);
      _this.isNew = false;
    }
    _this.getListData()
    _this.fileData = this.$defines.overtimeRequestFileData;
    _this.imgs = this.$defines.overtimeRequestImges;
    if (
