    ...
    <template v-if="isNew">
      ...
      >
        ...
        <div style="padding-top:10px">
          <textarea type="text" class="leaveReasons" placeholder="请输入外出地点" v-model="outGingAddress"></textarea>
        </div>
      </div>
      <div
        type="primary"
...
export default {
  ...
  methods: {
    //监听返回按钮
    goBack() {
      this.$defines.setOutRequestImges([]);
      this.$defines.setOutRequestFileData([]);
      this.$router.push({
        path: "/application",
        query: {
    ...
    readLocalFile: function() {
      ...
      reader.onload = function(event) {
        ...
        current.$defines.setOutRequestImges(current.imgs);
      };
    ...
    getListData() {
      if (!this.isNew) {
        ...
        let urls =
          ...
          "/user/searchApprovalOutGoing.do";
        _this.$ajax
          ...
          .then(function(response) {
            ...
            _this.approvalList = returnData.data.outGoingRecordList;
          });
      }
    },
    getDay(date, num) {
    ...
    getAttendanceRecord() {
      ...
      for (let i = 0; i < this.$defines.outRequestFileData.length; i++) {
        let file = this.$defines.outRequestFileData[i].files[0];
  ...
  created: function() {
    ...
    if (this.$route.query.pagename == "outRequestInformation") {
      localStorage.setItem("outRequestApproved", false);
      _this.isNew = false;
    }
    _this.getListData();
