<template>
  <div class="overtimeRequestApproval" ref="overtimeRequestApproval">
  ...
  methods: {
    ...
    goBack() {
      ...
      this.$router.push({
        path: "/management",
        query: {
          pagename: "overtimeRequestApproval"
        }
      });
    },
    //待审批
    pendingTrial() {
      ...
      if (localStorage.getItem("overtimeRequestapproved") == "false") {
        ...
        localStorage.setItem("overtimeRequestapproved", false);
        this.getListData();
      }
    },
    //已审批
    examined() {
      if (localStorage.getItem("overtimeRequestapproved") == "true") {
        return;
      } else {
        ...
        localStorage.setItem("overtimeRequestapproved", true);
        this.getListData();
      }
    },
    gotoScreening() {
      this.$router.push({
        path: "/overtimeRequestScreening",
        query: {
          pagename: "overtimeRequestApproval",
        }
      });
...
debugger
      var content = {
        ...
        bNum: 0, //从第几条开始
        rows: 20, //查询条数
        bt: localStorage.getItem("overtimeRequeststartTime")==null||localStorage.getItem("overtimeRequeststartTime")==""?this.getTIME(this.nowtime, 7):localStorage.getItem("overtimeRequeststartTime"), //开始时间
        et: localStorage.getItem("overtimeRequestendTime")==null||localStorage.getItem("overtimeRequestendTime")==""?this.getTIME(this.nowtime, 4):localStorage.getItem("overtimeRequestendTime"), //结束时间
        applicatUserId: localStorage.getItem("overtimeRequestApplicatUserId")==null?"":localStorage.getItem("overtimeRequestApplicatUserId"),
        approved: localStorage.getItem("overtimeRequestapproved")=="true"?true:false
      };
    ...
    changeFixed(clientHeight) {
      //动态修改样式
      this.$refs.overtimeRequestApproval.style.height = clientHeight + "px";
    }
  },
  mounted() {
     if (localStorage.getItem("overtimeRequestapproved") == "true") {
  ...
  created() {
    ...
    if (this.$route.query.pagename == "overtimeRequestDetails") {
      localStorage.setItem("overtimeRequestApproved", false);
    }
  ...
  watch: {
    ...
    clientHeight: function() {
      this.totalHeight = this.$refs.overtimeRequestApproval.offsetHeight;
