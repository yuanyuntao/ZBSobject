...
export default {
  ...
  methods: {
    ...
    goBack() {
      this.$router.push({
        path: "/outRequestApproval",
        query: {
          pagename: "outRequestDetails"
        }
      });
    },
    getURL(url) {
  ...
  created: function() {
    ...
    } else if (localStorage.getItem("outRequestapproved") == "true") {
      _this.approved = true;
