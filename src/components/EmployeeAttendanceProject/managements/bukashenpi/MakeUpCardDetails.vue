...
export default {
  ...
  methods: {
    ...
    goBack() {
      this.$router.push({
        path: "/makeUpCardApproval",
        query: {
          pagename: "makeUpCardDetails"
        }
      });
    },
    getURL(url) {
    ...
    sure(){
      var content = {
        ...
        appealAttendanceId: this.item.appeal_attendance_id,
        appealAttendanceRecordAuditId: this.item.appealAttendanceRecordAudit_id,
      };
      ...
      let url =
        ...
        "/user/submitApprovalAppealAttendance.do";
      this.$ajax
